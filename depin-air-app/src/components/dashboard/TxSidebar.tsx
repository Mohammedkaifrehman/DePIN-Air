  ...
  return (
    ...
    >
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-accent-green uppercase tracking-tight">
                        BATCH #{tx.seq}
                      </span>
                      {tx.isSpike && (
                        <div className="bg-accent-red/20 text-accent-red text-[10px] font-bold px-1.5 py-0.5 rounded border border-accent-red/30">
                          SPIKE
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {tx.cityBreakdown.map((c) => (
                      <div
                        key={c.city}
                        className="text-[10px] font-bold uppercase px-2 py-1 rounded bg-white/5 border border-white/10 text-text-secondary"
                      >
                  ...
                  <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-text-muted mb-3">
                    <span className="group-hover:text-text-secondary transition-colors tabular-nums">{tx.sensorCount} Nodes</span>
                    <span className="group-hover:text-text-primary transition-colors tabular-nums">Avg {tx.avgAqi} AQI</span>
                  </div>
      ...
      <div className="p-4 bg-white/[0.02] border-t border-white/5 text-center">
         <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Protocol v2.0 Hyper</span>
      </div>
    </div>
  );
}
