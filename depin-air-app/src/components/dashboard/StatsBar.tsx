  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[2000] h-[52px] bg-bg-secondary/60 backdrop-blur-3xl border-b border-white/10 flex items-center px-6 lg:px-10 gap-16">
      ...
      <Link href="/" className="flex items-center gap-4 shrink-0 group no-underline">
        <div className="w-8 h-8 rounded-md flex items-center justify-center text-[10px] font-bold bg-white text-black shadow-[0_0_20px_rgba(255,255,255,0.2)] group-hover:scale-105 transition-transform shrink-0">
          DA
        </div>
        <span className="text-[10px] font-bold text-text-primary hidden lg:block whitespace-nowrap uppercase tracking-widest">
          DePIN-Air
        </span>
      </Link>
      ...
      <nav className="hidden lg:flex items-center gap-12 ml-auto">
        {navLinks.map(link => (
          <Link
            key={link.href}
            href={link.href}
            className={`text-[10px] font-bold uppercase tracking-widest transition-all hover:scale-105 ${pathname === link.href ? 'text-accent-cyan shadow-[0_8px_15px_-5px_rgba(0,245,255,0.3)]' : 'text-text-muted hover:text-text-primary'}`}
          >
            {link.label}
          </Link>
        ))}
      ...
      {/* Status Bar */}
      <div className="hidden xl:flex items-center gap-2.5 shrink-0 px-4 py-1.5 rounded-sm bg-white/5 border border-white/10 text-white text-[10px] font-bold uppercase tracking-widest">
